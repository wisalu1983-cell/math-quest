...
7. 如果功能存在已确认 UI / 交互预览原型（如 `?preview=*`、HTML 原型、截图、设计稿或高保真分支），必须把原型列为 Test Basis；最终验收仍以开发落地版本或真实游戏流为证据，预览页只作对照依据
...
凡功能存在已确认 UI / 交互预览原型，L1 及以上 QA 必须增加原型一致性核对环节。

1. 测试方案必须列出原型来源、批准状态和适用范围，例如 `?preview=*` 页面、`.ui-design/preview/*.html`、截图、设计稿或预览分支。
2. 用例或 execution matrix 必须逐项覆盖原型中的关键状态、布局、交互步骤、焦点 / 输入规则、显隐时机、反馈文案、视觉 token 和响应式边界。
3. 每一项必须记录“原型期望 → 开发落地版本观察 → Evidence → Result”；不能只写“与原型一致”。
4. 最终证据必须来自真实开发落地版本或真实游戏流。预览页、静态截图、故事页只能作为 oracle / 对照依据，不能替代最终验收。
5. 若开发版本有意偏离原型，必须链接对应 Plan / Spec / 用户裁决，并在 qa-summary 的 Residual Risk 或 Scope Notes 中说明；未说明的 P0 / P1 偏离不得标 PASS。
...
- 已确认预览原型与开发落地版本逐项一致；任何有意偏离都有 Plan / Spec / 用户裁决依据
