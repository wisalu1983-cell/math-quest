...
async function runFreshUserBatch(page) {
  ...
  await runCase(page, {
    batch,
    id: 'B-08',
    title: 'Progress 与总体统计',
    expected: '进度页能展示累计答题与总体正确率。',
    steps: '从 summary 返回首页，再通过底部导航进入“进度”。',
    fn: async () => {
      await page.getByRole('button', { name: '回首页' }).click();
      await delay(300);
      await openBottomNav(page, '进度');
      const shot = await screenshot(page, 'B-08-progress.png');
      const body = await page.locator('body').innerText();
      const pass = body.includes('闯关进度') && body.includes('累计答题') && body.includes('总正确率');
      return {
        observed: `进度页标题=${body.includes('闯关进度')}；累计答题=${body.includes('累计答题')}；正确率=${body.includes('总正确率')}`,
        verdict: pass ? 'PASS' : 'FAIL',
        evidence: [shot],
      };
    },
  ...
  await runCase(page, {
    batch,
    id: 'B-09',
    title: 'History 与 SessionDetail',
    expected: '能进入练习历史并打开刚完成的练习详情。',
    steps: '在进度页点击“查看练习记录”，再打开第一条记录。',
    fn: async () => {
      await page.getByRole('button', { name: /查看练习记录/ }).click();
      await delay(300);
      const historyShot = await screenshot(page, 'B-09-history.png');
      await page.locator('button').filter({ hasText: '准确率' }).first().click();
      await delay(300);
      const detailShot = await screenshot(page, 'B-09-session-detail.png');
      const body = await page.locator('body').innerText();
      const pass = body.includes('练习详情') && body.includes('逐题详情') && body.includes('第 1 题');
      return {
        observed: `详情页标题=${body.includes('练习详情')}；逐题区=${body.includes('逐题详情')}；首题=${body.includes('第 1 题')}`,
        verdict: pass ? 'PASS' : 'FAIL',
  ...
  await runCase(page, {
    ...
    steps: '从练习详情返回首页，进入“我的”和“错题”。',
    fn: async () => {
      await page.getByRole('button', { name: '返回练习历史' }).click();
      await delay(200);
      await page.getByRole('button', { name: '返回进度页' }).click();
      await delay(200);
...
async function runAdvanceBatch(page) {
  ...
  await runCase(page, {
    ...
    fn: async () => {
      ...
      const body = await page.locator('body').innerText();
      const pass = body.includes('练习完成！') && body.includes('星级进度') && body.includes('答对题数');
      return {
        observed: `练习完成=${body.includes('练习完成！')}；星级进度=${body.includes('星级进度')}；答题统计=${body.includes('答对题数')}`,
        verdict: pass ? 'PASS' : 'FAIL',
        evidence: [shot],
      };
    },
  ...
  await runCase(page, {
    ...
    fn: async () => {
      ...
      const profileBody = await page.locator('body').innerText();
      const shortcutButton = page.locator('button').filter({ hasText: '题 →' }).first();
