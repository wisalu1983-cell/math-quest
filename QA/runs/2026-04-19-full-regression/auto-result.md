...
| A-04 | 浏览器全链脚本 | 新号/中盘/段位赛三条路径全部跑通且无 FAIL | `node QA/runs/2026-04-19-full-regression/full-regression.mjs` | 脚本执行完成并产出 3 份批次报告与截图；**Fresh 10/10 PASS，Advance 6/6 PASS，Rank 8/9 PASS**。唯一失败为 `D-07`：段位赛局内刷新后停在 `home`，未直接恢复到 `practice` | FAIL | `batch-1-fresh-user-result.md`<br>`batch-2-advance-result.md`<br>`batch-3-rank-match-result.md`<br>`artifacts/raw-results.json` |
