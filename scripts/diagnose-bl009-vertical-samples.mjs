...
function classifyOneDigitIntDivision(dividend, divisor, answer) {
  const quotient = Number(answer);
  if (!isInteger(dividend) || !isInteger(divisor) || !isInteger(quotient)) return null;
  if (digitCount(divisor) !== 1 || dividend % divisor !== 0) return null;
...
function classifyShape(operation, a, b, answer) {
  ...
  if (operation === '÷') {
    ...
    const load = classifyOneDigitIntDivision(a, b, answer);
    if (load) return `${left}÷${right}=${resultKind}:${load}`;
...
function classifyMentalCandidates(question) {
  ...
  if (operation === '÷' && isInteger(a) && isInteger(b) && digitCount(b) === 1 && isInteger(answer)) {
    const load = classifyOneDigitIntDivision(a, b, answer);
    if (load === 'D0') {
      candidateReasons.push('div-one-digit-D0');
    } else if (load === 'D1') {
      reviewReasons.push('div-one-digit-D1-transition');
    }
  }
