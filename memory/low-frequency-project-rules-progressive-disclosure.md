...
用户明确偏好：开新版本、版本生命周期这类低频项目管理规范，不要直接写进全局常读的项目管理文档；应采用渐进式披露，只在常用入口里放短链接，细则放独立低频文档。同时，优化不能只作用于当前已有文档，必须通过规则入口、模板或检查清单保证未来新版本也会按同一规则执行。

**Why:** 开新版本不是高频动作。如果把完整规范写进全局文档，会让每次会话或日常查规则时读到不相关内容，增加上下文噪音。

**How to apply:** 新增或调整低频项目管理规则时，优先创建独立指南文档；全局 `Overview.md` 不放；`Plan/README.md` 只放一行入口或速查链接。只有在处理版本启动、收口、结构诊断等触发场景时才读取低频指南。若规则影响未来工作流，必须同步更新模板、生命周期清单或机械检查，避免“当前修了，将来不知道”。
