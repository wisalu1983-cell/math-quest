...
> 所属版本：v0.1（原型，已发布）
> 归档日期：2026-04-20（随项目管理体系版本化迁移一并归档）
> 本文件角色：v0.1 收口瞬间从 `ProjectManager/ISSUE_LIST.md` 直接拷贝而来的**历史快照**，保留 ISSUE-001~064 的完整关闭记录与证据链。
>
> **路径说明**：
> - **Markdown 链接**（形如 `[文字](路径)`）已按当前文件位置（`ProjectManager/Plan/v0.1/issues-closed.md`）精准修复，可直接点击跳转。
> - **反引号代码描述**（形如 `` `Specs/xxx.md` ``、`` `src/pages/Practice.tsx` ``）保留原视角未改——`Specs/...`/`Reports/...`/`Plan/...` 以 `ProjectManager/` 根为基准，`src/...`/`test-results/...` 以 `math-quest/` 根为基准。这些是代码风格的引用说明，归档保持文字忠实而非重写。
>
> **与活跃视图的关系**：
> - `ProjectManager/ISSUE_LIST.md` v0.1 收口后已被精简为当前版本活跃视图
> - v0.1 收口瞬间仍开放的 `ISSUE-059` 已迁入 `ProjectManager/Backlog.md`（保留原 ID + 原始描述）
...
## 2026-04-16 试玩反馈修复（对应计划：Plan/v0.1/2026-04-16-p1p2-fixes.md）
...
## 2026-04-17 生成器题型设计 v2.1 重做（对应计划：Plan/v0.1/2026-04-17-generator-redesign-v2-implementation.md）
...
- **实施计划**: `Plan/v0.1/2026-04-17-generator-redesign-v2-implementation.md`（阶段 6：压档 + 细节修订）
...
- **关闭记录** (2026-04-17 子计划 2.5 S1-T1):
  ...
  - 详见 `Plan/v0.1/2026-04-17-campaign-advance-stabilization.md` §八 S1 执行结果
...
> 来源：`Plan/v0.1/2026-04-18-rank-match-phase3-implementation.md` M2 完工复盘（commit `205e35c`）。M2 主干已合入，但识别出两项遗留——一项是 Plan §4.1 明文风险未兑现验收（P1），一项是 Spec §5.6 要求的体验优化 M2 首版明确挂单为后续处理（P2）。两项都不阻塞 M3 UI 开工，但 `ISSUE-060` 阻塞 Phase 3 上线验收。
...
- **事实源**:
  - `Plan/v0.1/2026-04-18-rank-match-phase3-implementation.md` §4.1 第 173 行：
...
- **事实源**:
  ...
  - `Plan/v0.1/2026-04-18-rank-match-phase3-implementation.md` §6 M2 完工段第 293 行：
...
> 来源：`Plan/v0.1/2026-04-18-rank-match-phase3-implementation.md` M4 E2E QA（`test-results/phase3-rank-match/m4-user-qa-report.md`）。M3 已标完工但实际首次跑 `npm run build` + 拟真 E2E 时才暴露的两个 P1 问题；都在 M4 session 内当场修复闭环。未在 M3 漏网是因为 M3 仅跑了 `tsc --noEmit` + 手动截图核对，未做完整用户旅程 E2E。
...
- **关联**:
  ...
  - `ProjectManager/Plan/v0.1/2026-04-18-rank-match-phase3-implementation.md` §4.1 / §6
