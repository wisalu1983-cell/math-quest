...
| ID | 来源 | 摘要 | v0.4 初始归位 |
|---|---|---|---|
| `BL-005` | 4.25 真实用户反馈 | 竖式笔算体验问题集：颜色、进位格、乘法交互、难度、小数答案兼容 | Phase 1 + Phase 2 + Phase 3 |
| `BL-006` | 4.25 真实用户反馈 | 运算律填数字题槽位误导、缺操作说明 | Phase 3 |
| `BL-007` | 4.25 真实用户反馈 | 选项题只有 2 个选项，猜测概率过高 | Phase 2 |
| `BL-008` | 4.25 真实用户反馈 | 闯关题目重复，需先诊断再定性 | Phase 2 |
| `BL-003` | v0.2 收口 QA | compare 概念题方法提示补证 | Phase 3 |
| `BL-004` | v0.2 收口 Code Review | Practice 答题页状态重置实现清理 | Phase 4 |
...
2. `BL-008` 重复题目：先做诊断脚本还是直接从特定用户场景复现？
3. `BL-004` Phase 4：保留在 v0.4，还是在前三个 Phase 工作量偏大时延期到 v0.5？
