...
5. `BL-005.2` 进位/退位格三档规则：采用策略判定器方案；低档过程错不通过，中档答案对即通过且只在统一结果判定 UI 给当前题过程提示，高档不显示过程格。入口：[`subplans/2026-04-26-phase4-进位退位格规则与compare-tip补证.md`](./subplans/2026-04-26-phase4-进位退位格规则与compare-tip补证.md)

## 待决策项

1. `BL-004` Phase 5：工程方案已定；代码实施仍需等 Phase 4 最终验收稳定后启动，若 v0.4 收口节奏不允许，可延期到 v0.5。
