...
3. A04「运算律」和 A06「括号变换」取消玩家独立大题型入口，降阶为 A07「简便计算」低档知识点 lane。
   - 原 A04/A06 子题型能力不删除，作为 A07 lane 内部训练内容保留。
   - 玩家可见题型、进阶入口和段位赛题型范围同步改为 6 个主题型。
   - 旧 `operation-laws` / `bracket-ops` 数据保留为 internal / legacy source，不清空历史存档。

## 待决策项

1. `BL-005.2` 进位格三档规则：简单 / 中档 / 高档各自如何判定和反馈？
2. `BL-008` 重复题目：Phase 2 后先做诊断脚本还是直接从特定用户场景复现？
3. `BL-004` Phase 5：保留在 v0.4，还是在前四个 Phase 工作量偏大时延期到 v0.5？
