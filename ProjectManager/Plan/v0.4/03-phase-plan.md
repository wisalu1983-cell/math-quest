...
|---|---|---|---|---|---|
| Phase 1 | 渲染与判定修复 | 颜色、答案兼容、乘法竖式统一、`ISSUE-059` | 读 A03 相关代码 / 规格；按已确认三步路线展开乘法竖式子计划 | 对应手工或自动验证通过；test/build 通过 | 📋 待开工 |
| Phase 2 | 题目质量与生成器诊断 | 难度修复、选项干扰项、重复题目结论 | 读生成器 / 难度 / 星级规格；确定诊断样本 | 抽样验证有记录；重复题目有 bug/设计结论；test/build 通过 | 📋 待开工 |
...
Phase 1 可以最快落地；`BL-005.3` 已确认采用“多位整数乘法模块 → 小数乘法复用 → 小数点定位 / 答案等价”的路线，开工前需落独立子计划。Phase 2 牵涉生成器和抽题去重，必须先读规格并跑样本。Phase 3 包含教学反馈规则，必须在实现前先把预期体验说清楚。Phase 4 可作为 v0.4 末尾工程整理，也可在工作量超出时延期。
...
| 决策 | 影响文件 | 未决时处理 |
|---|---|---|
| 乘法竖式三步路线 | `phases/phase-1.md` / 子计划 | 已确认；开工前补子计划，明确多位整数模块、小数复用和小数点定位验收 |
