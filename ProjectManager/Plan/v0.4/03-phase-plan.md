...
| Phase 3 | 题目质量与生成器诊断 | 竖式难度、A03 `difficulty=4-5` 乘法分布、选项干扰项、重复题目结论 | Phase 2 后题型 IA 稳定；读生成器 / 难度 / 星级规格；复跑预研抽样 | 抽样验证有记录；重复题目有 bug/设计结论；test/build 通过 | 🟡 预研已收口，待复跑抽样后开工 |
...
- `BL-005.4` 竖式难度与 A03 `difficulty=4-5` 的 `2位数 × 2位数` 档位分布
- `BL-007` 其他题型选项干扰项设计
- `BL-008` 重复题目诊断
- `BL-005.2` 进位格规则
