...
> 最近调整：2026-04-26 Phase 5 工程方案定稿，代码实施闸门仍待 Phase 4 验收结论
...
| Phase 4 | 交互设计与教学引导 | 进位/退位格三档规则、compare tip 补证 | 进位/退位格规则已确认；compare tip 需重跑补证 | 用户视角走查 + 三档场景验证；compare tip 补证完成 | 🟡 最终验收中 |
| Phase 5 | Practice 工程质量 | 状态重置统一机制 | Phase 4 验收稳定；现有行为测试覆盖 | 重构前后行为等价；回归测试通过 | 🟡 工程方案已定 |
...
Phase 3：在新 IA 上做题目质量与重复题诊断（已完成）
  ↓
Phase 4：处理剩余教学交互规则（最终验收中）
  ↓
Phase 5：工程质量收尾（方案已定；代码实施可延期）
```
...
| Phase 5 去留 | `phases/phase-5.md` / `subplans/2026-04-26-phase5-Practice状态重置启动准备.md` | 工程方案已定；代码实施仍需 Phase 4 验收稳定后启动，可延期到 v0.5 |
...
- [`subplans/2026-04-26-phase5-Practice状态重置启动准备.md`](./subplans/2026-04-26-phase5-Practice状态重置启动准备.md)：Phase 5 Practice 状态重置工程方案

后续待展开：

- `BL-004` Practice 状态重置代码实施（闸门待确认）
