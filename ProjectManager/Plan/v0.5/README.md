...
> 所属版本：v0.5
> 创建：2026-04-28
> 状态：🟡 Phase 3 有条件完成；输入与反馈基础设施、自动换格统一化已实现并通过本地 QA，真实 Android / iOS 设备证据转发布后线上验收；下一步准备进入 Phase 4
...
- `subplans/`：Phase 1 开工对齐文档已完成；Phase 2 `BL-009` 子计划已完成；Phase 3 `BL-011`、`BL-011` 自动换格统一化与 `ISSUE-067` 子计划已完成；Phase 4 的详细设计与实施计划按启动时创建。
