...
- **一份联合诊断报告**：建议路径 `ProjectManager/Reports/2026-04-XX-phase-beta-diagnosis.md`（具体日期启动时定）
- 每项包含：
...
- [ ] Phase α 完成（F3 可用，利于快速造场景复现）

## 收尾条件

- [ ] 联合诊断报告落盘并通过用户审阅
- [ ] 每项归位建议已写入本主线的 γ / δ / 独立小修 之一
- [ ] 若任一项被定性为"设计本身不到位"，本文档追加"推迟到 δ"的标记

## 对后续 Phase 的影响

- **γ 内容完全由本阶段结论决定**，在 β 完成前 γ 只保持占位
- 若某项被定性为"设计不到位"，会直接推迟到 δ 一并处理，不进入 γ
