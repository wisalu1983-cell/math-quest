...
> 版本：v1.16 | 创建：2026-04-17 | 上次修订：2026-04-29（v0.5 Phase 3 回写内置键盘自动换格与乘法输入顺序约束）
...
| `a03-vertical-calc/current.md` | **生效（current spec；v0.5 Phase 4 已回写）** | A03 竖式板过程格显示 / 跳格 / 本地复盘 / 统一结果 UI / 单行竖式操作数高对比 / 低档竖式样本过滤 / 内置键盘固定底部 / 自动换格 / 多行乘法右到左输入 / 结构化错因反馈 / 长除法纸面逐位输入、逐步显现、小数 ÷ 小数扩倍阶段替换、步骤级错因和循环小数标准格式预览 |
