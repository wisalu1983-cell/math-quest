...
> 版本：v1.13 | 创建：2026-04-17 | 上次修订：2026-04-26（v0.4 release gate 后补充 A03 单行竖式高对比 current spec 约束）
...
| `a03-vertical-calc/current.md` | **生效（current spec；v0.4 已发布）** | A03 `vertical-fill` 当前权威行为：低档非 0 过程格必填且过程错不通过；中档答案对即通过且只给当前题过程提示；高档不显示过程格；单行竖式已知操作数 / 运算符使用高对比正文色；过程失败原因字段可选、不 bump 存档版本 |
...
| `a03-vertical-calc/current.md` | **生效（current spec；v0.4 已发布）** | A03 竖式板过程格显示 / 跳格 / 本地复盘 / 统一结果 UI / 单行竖式操作数高对比的当前行为入口 |
