...
> 最后更新：2026-04-30（新增 `BL-014` 乘法竖式迭代候选；新增 `BL-015` v0.6 开发 / QA 子 agent 试点候选；新增 `BL-016` master lint 债清理候选；新增 `BL-017` 题型生成器硬编码样例池审计候选；新增 `BL-018` 预览原型一致性 QA 门禁候选）
...
### BL-018 · 预览原型一致性 QA 门禁

- **来源**：2026-04-30 用户要求记录 QA 流程待办；既有证据包括 v0.4 乘法竖式预览页验收未覆盖真实 Practice 落地上下文，release gate 才暴露 `ISSUE-065`。
- **背景**：部分 UI / 交互功能会先做 `?preview=*`、HTML 预览、截图或高保真原型，再进入正式开发。若 QA 只测开发落地版本的功能可用性，未逐项对照预览原型，容易遗漏版式、阶段流、焦点顺序、输入反馈、视觉 token、响应式状态等原型已确认约束。
- **类别**：QA 制度 / UI 原型验收 / 预览到生产一致性
- **状态**：候选（已同步补入 QA Leader canonical，后续版本如需系统化可再扩展为模板和检查表）
- **初步分流**：在正式 QA 方案中增加“预览原型一致性”环节：凡功能存在已确认 UI 或交互预览原型，测试用例必须把原型列为 Test Basis，并建立“原型项 → 开发落地版本 → 证据 → 结论”的逐项核对记录；最终证据必须来自真实开发落地版本或真实游戏流，预览页只能作为对照依据。
- **期望方向**：QA 不只判断“功能能用”，还要确认正式版本与已确认预览原型一致；若存在有意偏离，必须写明对应 Plan / Spec / 用户裁决，不能静默通过。
