...
> 最后更新：2026-04-27（新增 BL-009 ~ BL-012 候选需求）
...
- **状态**：候选
- **初步分流**：后续激活时优先检查 A03 `vertical-calc` 生成器、campaign 关卡过滤条件、难度档位边界，以及 `ProjectManager/Specs/a03-vertical-calc/current.md` 对竖式训练目标的当前承诺。
...
- **类别**：新功能 / A03 竖式计算交互
- **状态**：候选
- **初步分流**：后续激活时需先确认现有除法题渲染、判定、输入模型与 `VerticalCalcBoard` / 多行竖式组件的边界，再决定是扩展现有 A03 竖式板，还是新增除法专用竖式答题组件。
...
- **来源**：2026-04-27 用户新需求反馈
- **背景**：需要提供内置键盘，仅包含计算题输入所需字符；移动端默认使用内置键盘，不自动弹出系统键盘；系统键盘可由玩家手动呼叫。
- **类别**：新功能 / 输入体验 / 移动端 UX
- **状态**：候选
- **初步分流**：后续激活时需评估所有计算题输入入口，包括普通答案输入、竖式格输入、负数 / 小数 / 分数 / 运算符输入，以及移动端浏览器对系统键盘抑制、焦点管理和可访问性的限制。
- **期望方向**：计算输入默认走稳定、低干扰、题型定制的内置键盘；保留手动呼叫系统键盘作为兼容路径。
...
- **类别**：新功能 / 内部后台 / 账号数据运维
- **状态**：候选
- **初步分流**：后续激活时需对齐 v0.3 Supabase 账号同步规格，明确可查询的数据表、账号定位方式、访问权限、生产环境入口、隐私边界、审计要求与只读保护。
