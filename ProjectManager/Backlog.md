...
### BL-024 · 答错结算界面显示用户填写的错误回答

- **来源**：2026-05-03 用户反馈
- **背景**：目前答错结算界面只显示正确答案和提示，不显示用户实际填写的错误内容。用户无法对比自己的回答和正确答案，不利于定位错误根源。
- **类别**：体验优化 / 答错反馈 / 学习闭环
- **状态**：候选
- **初步分流**：需确认哪些题型的结算界面受影响（普通填空、选项题、竞式题、长除法等）；设计展示方式（用户错误回答 vs. 正确答案对比）；评估是否与已有的结构化错因 `failureDetail` 合并设计。
- **期望方向**：答错结算界面同时展示“你填的是 XXX”和“正确答案是 YYY”，让用户能直接看到差距在哪里；不改变当前错题本的展示逻辑。
