...
> 最后更新：2026-05-01（完成 `BL-017` v0.5 审计判断与 `cyclic-div` 小范围 hardening，剩余系统性样例池治理回流 v0.6）
...
- **状态**：v0.5 Release Gate 审计判断与 `vertical-calc/cyclic-div` 小范围 hardening 已完成；剩余系统性样例池治理延期至 v0.6（版本包待启动）。
- **v0.5 处理结果**：Phase 5 已审计 `src/engine/generators/` 下各题型有限样例池；确认 `operation-laws`、`number-sense/floor-ceil`、`decimal-ops`、`mental-arithmetic/order` 等旧题型存在系统性模板池重复风险，不适合在 v0.5 Release Gate 内全量重写。当前版本只修复新增发布面 `vertical-calc/cyclic-div`：从 3 个固定样例改为受约束循环小数生成，原 3 例保留为兜底；240 题审计从 3 个唯一题干提升到约 204 个唯一题干。
- **后续分流**：v0.6 启动时建立独立生成器样本质量优化任务，按题型拆分硬编码池、有限枚举池、fallback 样例和 session 重复率治理；优先处理本次审计中重复率最高且用户高频遇到的概念 / 情境题。
